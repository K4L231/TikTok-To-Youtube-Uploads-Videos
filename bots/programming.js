...
(async () => {
  ...
  await page.goto('https://www.tiktok.com/tag/programming?lang=en', {
      waitUntil: 'networkidle0',
      timeout: 0
    });
  const fs = require('fs');
  const jsonString = fs.readFileSync("./json/programming.json");
  ...
  console.log(data.length)
    ...
    for(let i = 0; i < (data.length * 10) + 100; i = i + 10){
      let vidInfo = await page.evaluate((i) => {
        let vidLink = document.querySelectorAll('.tiktok-1gs7l3h-DivItemContainer.e1z53d07')[i].querySelector('a').href
    ...
    async function pushData(title, link){
      ...
      data.push(title);
      fs.writeFile ("./json/programming.json", JSON.stringify(data), function(err){
          if (err) throw err;
    ...
    await page3.keyboard.type("kazkonoretusi@gmail.com");
