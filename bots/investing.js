...
(async () => {
  ...
  await page.goto('https://www.tiktok.com/tag/investing?lang=en', {
      waitUntil: 'networkidle0',
      timeout: 0
    });
  const fs = require('fs');
  const jsonString = fs.readFileSync("./json//investing.json");
  ...
  console.log(data.length)
    ...
    for(let i = 0; i < (data.length * 10) + 100; i = i + 1){
    ...
    async function pushData(title, link){
      ...
      data.push(title);
      fs.writeFile ("./json//investing.json", JSON.stringify(data), function(err){
          if (err) throw err;
    ...
    await page3.keyboard.type("lolasZjbsYr@gmail.com");
